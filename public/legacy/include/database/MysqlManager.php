...
class MysqlManager extends DBManager
{
    ...
    {
        parent::getLog()->debug('Calling MySQL::disconnect()');
    ...
    public function limitQuery($sql, $start, $count, $dieOnError = false, $msg = '', $execute = true)
    {
        ...
        parent::getLog()->debug('Limit Query:' . $sql . ' Start: ' . $start . ' count: ' . $count);
        ...
        foreach ($badQuery as $table => $data) {
            if (!empty($data)) {
                ...
                if (!empty($GLOBALS['sugar_config']['check_query_log'])) {
                    parent::getLog()->fatal($sql);
                    parent::getLog()->fatal('CHECK QUERY:' . $warning);
                } else {
                    parent::getLog()->warn('CHECK QUERY:' . $warning);
                }
            }
        }
    ...
    public function connect(array $configOptions = null, $dieOnError = false)
    {
        ...
        if (!$this->database) {
            ...
            if (empty($this->database)) {
                parent::getLog()->fatal("Could not connect to server " . $configOptions['db_host_name'] . " as " . $configOptions['db_user_name'] . ":" . mysql_error());
                if ($dieOnError) {
        ...
        if (!empty($configOptions['db_name']) && !@mysql_select_db($configOptions['db_name'])) {
            parent::getLog()->fatal("Unable to select database {$configOptions['db_name']}: " . mysql_error($this->database));
            if ($dieOnError) {
                sugar_die($GLOBALS['app_strings']['ERR_NO_DB']);
            } else {
                return false;
	...
	if(!empty($charset)) {
        ...
        if (!$this->checkError('Could Not Connect:', $dieOnError)) {
            parent::getLog()->info("connected to db");
        }
        $this->connectOptions = $configOptions;

        parent::getLog()->info("Connect:" . $this->database);
